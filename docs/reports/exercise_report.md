...
混同行列（`data/artifacts/visualizations/latest/confusion_matrix.png`）から以下のことがわかります：
...
SHAP summary plot（`data/artifacts/visualizations/latest/shap_summary_plot.png`）から以下のことがわかります：
...
特徴量重要度プロット（`data/artifacts/visualizations/latest/shap_feature_importance.png`）から以下のことがわかります：
...
依存関係プロット（`data/artifacts/visualizations/latest/dependence_plot_*.png`）から以下のことがわかります：
